...
DELIMITER ;;
/*!50003 CREATE*/ /*!50017 DEFINER=`root`@`localhost`*/ /*!50003 TRIGGER `ebooks_BEFORE_INSERT` BEFORE INSERT ON `ebooks` FOR EACH ROW BEGIN
	DECLARE time_difference integer;
    
    SELECT YEAR(authors.date_of_birth) - NEW.published_year 
    INTO time_difference
    FROM authors
    WHERE NEW.author_id = authors.id;
    
    IF time_difference > 0 THEN
		signal sqlstate '45000' set message_text = 'Author cannot write books before born';
	END IF;
END */;;
DELIMITER ;
...
/*!50003 SET sql_mode              = 'STRICT_TRANS_TABLES,NO_ENGINE_SUBSTITUTION' */ ;
DELIMITER ;;
/*!50003 CREATE*/ /*!50017 DEFINER=`root`@`localhost`*/ /*!50003 TRIGGER `ebooks_BEFORE_UPDATE` BEFORE UPDATE ON `ebooks` FOR EACH ROW BEGIN
	DECLARE time_difference integer;
